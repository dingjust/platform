...
class SalesOrderForm extends StatefulWidget {
  ...
  SalesOrderForm({@required this.quoteModel});
...
//是否开具发票
  ...
  void onSure() async {
    bool isSubmit = false;
    List<SalesOrderEntryModel> entries = new List();
    //生产总数
    salesOrder.totalQuantity = _totalQuantity;
    //备注
    ...
    if (productModel != null &&
      ...
      for (int i = 0; i < productModel.variants.length; i++) {
        SalesOrderEntryModel entryModel = new SalesOrderEntryModel();
    ...
    //单价
    if (price != null) {
      salesOrder.unitPrice = double.parse(price);
    } else {
      salesOrder.unitPrice = widget.quoteModel.unitPrice;
    }
    salesOrder.entries = entries;

    salesOrder.salesApplication = SalesApplication.ONLINE;

    salesOrder.quoteOrderCode = widget.quoteModel.code;

    try {
      //非空验证
      if (salesOrder.entries == null || salesOrder.entries.length <= 0) {
        isSubmit = _showValidateMsg(context, '请选择产品');
      } else if (salesOrder.totalQuantity == null ||
          salesOrder.totalQuantity <= 0) {
        isSubmit = _showValidateMsg(context, '请输入生产数量');
      } else if (salesOrder.unitPrice == null || salesOrder.unitPrice < 0) {
  ...
  void onSubmit(SalesOrderModel salesOrder) async {
    Function canFunc = BotToast.showLoading(
      crossPage: false,
      clickClose: true,
    );
    CommonResponse response =
        await SalesOrderRespository().orderByProduct(salesOrder,params: {"isQuoteSource":true});
    canFunc();
    if (response.code == 1) {
      BotToast.showText(text: "创建销售订单成功");
      if(response.data != null && response.data is List && response.data.length > 0){
        getPurchaseOrderDetail(response.data[0]);
      }

    } else {
      BotToast.showText(text: "创建销售订单失败");
    }
//
//    showDialog(
//        context: context,
...
//            requestCallBack: SalesOrderRespository().orderByProduct(salesOrder),
...
//    });
  ...
  void getPurchaseOrderDetail(String code) async {
    Navigator.of(context).pushAndRemoveUntil(
        MaterialPageRoute(
            builder: (context) => SaleOrderDetailPage(code: code)),
        ModalRoute.withName('/'));
