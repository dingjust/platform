    ...
    if(widget.item.colorSizes != null && widget.item.colorSizes.length > 0){
      _selectColors = widget.item.colorSizes.map((color){
        ...
        if(color.customize != null && color.customize){
          if(_colors.indexWhere((color1) => color1.colorName == color.colorName) < 0){
            _colors.add(color);
          }
        }
        if(color.sizes != null && color.sizes.length > 0 && _selectSizes.length <= 0){
      ...
      print('initState----------------');
      _selectColorNames = _selectColors.map((color)=>color.colorName).toList();
    }
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          centerTitle: true,
          actions: <Widget>[
            IconButton(icon: Text('确定'), onPressed: (){
              ...
              child: Row(
                ...
                children: <Widget>[
                  ...
                  FlatButton(
                    ...
                    child: Row(
                      children: <Widget>[
                        Icon(Icons.restore_from_trash,size: 17,),
                        Text('清空已选项'),
                      ],
                   ))
                ],
              ...
              child: Row(
                children: <Widget>[
                  FlatButton(
                      ...
                      child: Row(
                        children: <Widget>[
                          Icon(Icons.add,size: 17,color: Colors.red,),
                          Text('自定义',style: TextStyle(color: Colors.red,),),
                        ],
                      )
                  ),
                ],
              ...
              return Container(
                child: ChoiceChip(
                  ...
                  shape: RoundedRectangleBorder(
                    ...
                    side: BorderSide(color: _selectSizeNames.contains(_sizes[index].name)?Colors.white:Colors.grey),
                  ),
                  ...
                  onSelected: (value) {
                    setState(() {
                      if (value) {
                        ...
                        _selectSizes.add(ColorSizeEntryModel(code: _sizes[index].code,name: _sizes[index].name));
                      } else {
                        _selectSizeNames.remove(_sizes[index].name);
                        _selectSizes.removeWhere((size) => _sizes[index].name == size.name);
                      }
                    });
                  },
                ),
...
//                        autofocus: true,
                ...
                FlatButton(
                    onPressed: (){
                      setState(() {
                        if(_customColorController.text != ''){
                          ColorSizeModel color = ColorSizeModel(colorName: _customColorController.text,customize: true);
                    ...
                    child: Text('添加',style: TextStyle(color: Colors.red,),)
                ),
                ],
              ),
            ),
                    ...
                    onPressed: (){
                      setState(() {
                        if(_customSizeController.text != ''){
                          ColorSizeEntryModel size = ColorSizeEntryModel(name: _customSizeController.text,customize: true);
              ...
              child: GridView.count(
                ...
                children: List.generate(_colors.length,
                      ...
                      return ChoiceChip(
                        ...
                        shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(5),
                          side: BorderSide(color: _selectColorNames.contains(_colors[index].colorName)?Colors.white:Colors.grey),
                        ),
                        backgroundColor: Colors.white,
                        avatar: null,
                        selectedColor: Color(0xffffd60c),
//                                    key: ValueKey<String>(_colors[index].name),
                        ...
                        selected: _selectColorNames.contains(_colors[index].colorName),
                        onSelected: (value) {
                          setState(() {
                            if (value) {
                              _selectColorNames.add(_colors[index].colorName);
                              _selectColors.add(ColorSizeModel(colorName: _colors[index].colorName,colorCode: _colors[index].colorCode));
                            } else {
                              _selectColorNames.remove(_colors[index].colorName);
                              _selectColors.removeWhere((color) => _colors[index].colorName == color.colorName);
                            }
                          });
                        },
                      );
