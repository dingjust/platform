...
import style from "components/Customization/CustomizationComponentsList.less"
import { SearchBar,Tabs,ListView,Modal, Toast } from 'antd-mobile';
...
class QueryCondition extends React.Component{
    imgItemClass=(state)=>{
        if(state.checked){
            return `${style.imgItem} ${style.bigSize}`
        }else{
            return `${style.imgItem}`
        }
    }
    itemClick=(index,key)=>{
        if(key == 0){
                ...
                <div className={`flex1`}>
                    <div className={style.imgBox}>
                        {this.props.selectList.map((imgList,index)=>(
                            <div className={this.imgItemClass(imgList)}>
                                {imgList.checked?
...
class CustomizationComponentsList extends React.Component {
    state={
        ...
        detailsState:true,
        returnList:true,
        dataSource:dataSource1.cloneWithRows(this.props.list)
    }
    hei = document.documentElement.clientHeight - 500;

    selectBg=(code,key)=>{
        if(code==this.props.list[key].code){
            return `selectBg`
        }
    }
    ...
    row = (rowData, sectionID, rowID) => {
        return (
          <div key={rowID} className={`${style.selectListBox} ${this.selectBg(this.props.part,rowID)}`}>
                <div 
                onClick={()=>{
                    this.props.alterComponents(
                    rowData,
...
const mapStateToProps=(state)=> {
    const {colorArray,textureArray,filtrate} = state.shoppingCart;
    return {
      colorArray,
      textureArray,
      filtrate
...
const mapDispatchToProps = (dispatch) => {
    return {
        alterComponents: async(component,index,listIndex,entryNumber) =>{
          let state = await dispatch({ type: 'shoppingCart/alterComponents',payload:{component,index,listIndex,entryNumber}});
            if(!state){
                Toast.fail('修改失败！');
            }
        },
        selectColor:(index)=>{
            dispatch({type:'shoppingCart/selectColor',payload:{index}})
        },
        selectTexture:(index)=>{
            dispatch({type:'shoppingCart/selectTexture',payload:{index}})
        },
        getCustomizationList:( )=>{
            dispatch({type:'shoppingCart/getCustomizationList',payload: { productCode:``, absPartCode:`` } })
        },
        getTexture:()=>{
            dispatch({type:`shoppingCart/getTexture`})
        },
        getDetail:(partCode)=>{
            dispatch({type:`shoppingCart/getDetail`,payload:{partCode}})
        }
    }
  }
