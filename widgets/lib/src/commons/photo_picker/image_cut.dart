...
class ImageCutPage extends StatefulWidget {
  ImageCutPage({Key key, this.title,this.file}) : super(key: key);
  final String title;
  final File file;
  @override
...
class _ImageCutPageState extends State<ImageCutPage> {
  ...
  void initState() {
    ...
    _points.add(Offset(0, 0));
    _points.add(Offset(0, 0));
    ImageLoader.load(widget.file.path).then((image2) {
      setState(() {
        this.image = image2;
      });
    });
          ...
          child:Row(children: <Widget>[
            RaisedButton(
                ...
                onPressed: ()async{
                  ...
                  flutterui.Image image2 = await recorder.endRecording().toImage(dest.width.toInt(), dest.height.toInt());
                  Navigator.pop(context,image2);
