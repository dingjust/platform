...
<script>
  ...
  import CollectionsDetailsPage from './CollectionsDetailsPage';
  ...
  export default {
    ...
    methods: {
      ...
      onNew() {
        this.fn.openSlider('添加图片', CollectionsForm, {
          name: '',
          description: '',
          active: true,
          action: '',
      ...
      async _onSearch(page, size) {
        ...
        const result = await this.$http.get('/djbackoffice/system/collections', params);
        if (result["errors"]) {
      ...
      async changeShelfStatus(row) {
        ...
        let message = '启用';
        let url = '/djbackoffice/system/collections/enable/';
        if (row.active === false) {
          message = '禁用';
          url = '/djbackoffice/system/collections/disable/';
        }
