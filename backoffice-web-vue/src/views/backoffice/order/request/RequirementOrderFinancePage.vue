    ...
    <el-card>
      ...
      <el-table v-if="isHeightComputed" ref="resultTable" stripe :data="page.content" @filter-change="handleFilterChange" :height="autoHeight">
        <el-table-column label="订单编号" prop="code" width="250">
          ...
          <template slot-scope="scope">
            <span>{{scope.row.code}}</span>
            <el-tag v-show="isBeforeApproved(scope.row.status)" disable-transitions>
              待审核
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="订金" prop="depositPaid">
          <template slot-scope="scope">
            <el-tag
              :type="scope.row.depositPaid ? 'success' : ''"
              disable-transitions>{{scope.row.depositPaid ? '已付' : '未付'}}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label="尾款" prop="retainagePaid">
          <template slot-scope="scope">
            <el-tag
              :type="scope.row.retainagePaid ? 'success' : ''"
              disable-transitions>{{scope.row.retainagePaid ? '已付' : '未付'}}
            </el-tag>
          </template>
        </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      _onSearch(page, size) {
        axios.post("/djbackoffice/requirementOrder?page=" + page + "&size=" + size, {
          code: this.text,
      ...
      isBeforeApproved(status) {
        // 财务确认前的状态都要显示
        return ['WAIT_FOR_PROCESSING', 'PENDING_APPROVAL', 'APPROVED'].includes(status);
      },
