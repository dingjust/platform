  ...
  <div class="animated fadeIn content">
    <el-card>
      ...
      <el-row>
        ...
        >
          <el-tab-pane label="顶部轮播(小程序)" name="FACTORY_CAROUSEL" />
          <el-tab-pane label="顶部轮播(APP)" name="BRAND_CAROUSEL" />
        ...
        <promote-product-carousel
          ref="carousel"
          v-if="activeName == 'SEE_PRODUCT_CAROUSEL'"
          @operationCount="operationCount"
          @returnCount="returnCount"
        />
        <promote-product-factory-carousel
          v-if="activeName == 'FACTORY_CAROUSEL'"
          @operationCount="operationCount"
          @returnCount="returnCount"
        />
        <promote-product-brand-carousel
          v-if="activeName == 'BRAND_CAROUSEL'"
          @operationCount="operationCount"
          @returnCount="returnCount"
        />
        <promote-product-today
          ref="today"
          v-if="activeName == 'TODAY_NEW'"
          @onConfirmToday="onConfirmToday"
          :formData="formData"
          @operationCount="operationCount"
          @returnCount="returnCount"
        />
        <promote-product-season
          ref="season"
          v-if="activeName == 'SEASON_HOT'"
          :formData="formData"
          :slotData="promoteProductList"
          @onProuductSelect="onProuductSelect"
          @onConfirm="onConfirm"
          @operationCount="operationCount"
          @returnCount="returnCount"
        ...
        />
        <promote-product-live
          ref="live"
          v-if="activeName == 'LIVE_BROADCAST_PROVIDE'"
          :formData="formData"
          :slotData="promoteProductList"
          @onProuductSelect="onProuductSelect"
          @onConfirm="onConfirm"
          ...
          @_moveNumber="_moveNumber"
        />
        <promote-product-for-you
          ref="forYou"
          v-if="activeName == 'RECOMMEND_FOR_YOU'"
          :formData="formData"
          :slotData="promoteProductList"
          @onProuductSelect="onProuductSelect"
          @onConfirm="onConfirm"
...
export default {
  ...
  methods: {
    ...
    tabBeforeLeave(nval, oval) {
      let flag;
      if (
        this.carouselEnum.indexOf(this.activeName) > -1 ||
        this.activeName === "TODAY_NEW"
      ) {
    ...
    async getProductPlate(activeName) {
      ...
      if (
        this.carouselEnum.indexOf(activeName) < 0 &&
        activeName != "TODAY_NEW"
      ) {
        this.$store.state.PromoteProductModule.promoteProductList =
          Object.assign([], this.formData.sequenceProducts);
    ...
    onListSearch(keyword) {
      if (keyword.replace(/\s*/g, "").length == 0) {
        ...
        const searchData = this.originData.filter(function (item) {
          return (
            item.product.name.search(keyword) > -1 ||
            item.product.skuID.search(keyword) > -1
          );
        });
    ...
    _moveNumber(index, modifyText, keyword) {
      const modifyT =
        modifyText - this.originData.length >= 0
