    ...
    <el-aside width="73px">
      <div class="dialog-aside_buttons">
        <el-button-group>
          <el-button v-if="
              slotData.state == 'SIGN' ||
              slotData.state == 'PARTY_A_SIGN' ||
              slotData.state == 'PARTY_B_SIGN'
            " icon="el-icon-edit" @click="onSearchSeal">签署</el-button>
          <el-button icon="el-icon-share" @click="onDownload(slotData.code)">下载</el-button>
          <el-button v-if="
              slotData.state == 'SIGN' ||
              slotData.state == 'PARTY_A_SIGN' ||
              slotData.state == 'PARTY_B_SIGN'
            " icon="el-icon-delete" @click="onRefuse()">拒签</el-button>
        </el-button-group>
      </div>
    </el-aside>
  </el-container>
    ...
    methods: {
      ...
      async onDownload(code) {
        ...
        window.location.href =
          "http://localhost:8081/b2b/user/agreement/download/" + result.data;
      },
