var CDS = {
    ...
    companySeal: function (company, cType, fType, sType, seaNo) {
        ...
        function writeSerNo(serNo) {
            context.translate(width, height);
            context.font = 'bolder 8px ' + CDS.baseConf.font;
    ...
    personal: function (name, cType, fType, type, border) {
        this.commonMethod(cType, fType);
        var color = CDS.baseConf.color;
        var font = CDS.baseConf.font;
        //获取canvans doc对象
        //var canvas = document.getElementById('canvas');
        var canvas = document.createElement("canvas");
        var context = canvas.getContext('2d');
    ...
    //椭圆签章
    companyEllipse: function (company, serNo, fType, cType, sType) {
        ...
        function writeTitle() {
            ...
            context.fillText(sType, radiusX, radiusY + 20);
            context.restore();
        }

        function drawEllipse() {
            context.ellipse(radiusX + context.lineWidth + 1, radiusY + context.lineWidth + 1, radiusX, radiusY,
                0, 0, Math.PI * 2);
            //背景透明
    ...
    wrapText: function (ctx, text, x, y, maxWidth, lineHeight) {
        ...
        if (typeof lineHeight == 'undefined') {
            lineHeight = (canvas && parseInt(window.getComputedStyle(canvas).lineHeight)) || parseInt(window.getComputedStyle(
                document.body).lineHeight);
        }
