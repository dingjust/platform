  ...
  <div>
    ...
    <el-table :data="tableData" border ref="table">
          ...
          <div v-else>
            <el-button type="text" @click="onSelectProduct(scope.$index)">
              <div v-if="
                  ...
                  scope.row.product.id !== ''
                ">
                <img v-if="scope.row.product.thumbnail" :src="scope.row.product.thumbnail.url"
      ...
      <el-table-column label="交货时间" prop="deliveryDate" min-width="160px">
        <template slot-scope="scope" v-if="!scope.row.countRow">
          <el-date-picker v-model="scope.row.deliveryDate" type="date" value-format="timestamp" style="width: 130px"
            placeholder="选择日期">
          </el-date-picker>
        </template>
      </el-table-column>
      <el-table-column label="合同时间" prop="contractDate" min-width="160px">
        <template slot-scope="scope" v-if="!scope.row.countRow">
          <el-date-picker v-model="scope.row.contractDate" type="date" value-format="timestamp" style="width: 130px"
            placeholder="选择日期">
          </el-date-picker>
        </template>
      </el-table-column>
      ...
      <el-table-column label="合同单价（不含税）" prop="unitContractPrice" min-width="120px">
        <template slot-scope="scope">
          <el-input v-model="scope.row.unitContractPrice" v-if="!scope.row.countRow"
            @change="inputChange(scope.$index, 'unitContractPrice')" v-number-input.float="{ min: 0, decimal: 2 }">
          </el-input>
      ...
      <el-table-column label="货款金额" prop="loanAmount" min-width="120px">
        <template slot-scope="scope">
          <el-input v-if="!scope.row.countRow" v-model="scope.row.loanAmount"
            @change="inputChange(scope.$index, 'loanAmount')" v-number-input.float="{ min: 0, decimal: 2 }"></el-input>
          <span v-else>{{ scope.row.loanAmount }}</span>
        </template>
      </el-table-column>
      <el-table-column label="快递费" prop="expressFee" min-width="120px">
        <template slot-scope="scope">
          <el-input v-if="!scope.row.countRow" v-model="scope.row.expressFee"
            @change="inputChange(scope.$index, 'expressFee')" v-number-input.float="{ min: 0, decimal: 2 }"></el-input>
          <span v-else>{{ scope.row.expressFee }}</span>
        </template>
      </el-table-column>
      <el-table-column label="扣款" prop="deductionAmount" min-width="120px">
        <template slot-scope="scope">
          <el-input v-if="!scope.row.countRow" v-model="scope.row.deductionAmount"
            @change="inputChange(scope.$index, 'deductionAmount')" v-number-input.float="{ min: 0, decimal: 2 }">
          </el-input>
      ...
      <el-table-column label="结算金额" prop="settlementAmount" min-width="120px">
        <template slot-scope="scope">
          <el-input v-if="!scope.row.countRow" v-model="scope.row.settlementAmount"
            @change="inputChange(scope.$index, 'settlementAmount')" v-number-input.float="{ min: 0, decimal: 2 }">
          </el-input>
      ...
      <el-table-column label="定金" prop="depositAmount" min-width="120px">
        <template slot-scope="scope">
          <el-input v-if="!scope.row.countRow" v-model="scope.row.depositAmount"
            @change="inputChange(scope.$index, 'depositAmount')" v-number-input.float="{ min: 0, decimal: 2 }">
          </el-input>
      ...
      <el-table-column label="备注" prop="remark" min-width="225px">
        <template slot-scope="scope" v-if="!scope.row.countRow">
          <el-input v-model="scope.row.remarks" type="textarea" style="width: 200px" :rows="3"
            :title="scope.row.remarks"></el-input>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="importVisible" width="80%" class="purchase-dialog" append-to-body
      :close-on-click-modal="false">
      <reconciliation-entry-import v-if="importVisible" @onImport="onImport" />
    </el-dialog>
    <el-dialog :title="columnDialogtitle" :visible.sync="columnVisible" width="400px" append-to-body
      :close-on-click-modal="false">
      <append-column v-if="columnVisible" :modifyColumn="modifyColumn" @toModifyColumn="_modifyColumn"
        @addColumn="addColumn" @callback="callback" />
    </el-dialog>
  </div>
</template>
    ...
    computed: {
      tableData: function () {
        ...
        if (this.formData.entries.length > 0) {
          data.push({
            countRow: "合计",
            orderQuantity: this.countColumn(
              this.formData.entries,
              "orderQuantity"
            ),
            // cutQuantity: this.countColumn(this.formData.entries, 'cutQuantity'),
            packageQuantity: this.countColumn(
              this.formData.entries,
              "packageQuantity"
            ),
            storageQuantity: this.countColumn(
              this.formData.entries,
              "storageQuantity"
            ),
            ...
            depositAmount: this.countColumn(
              this.formData.entries,
              "depositAmount"
            ),
            settlementAmount: this.countColumn(
              this.formData.entries,
              "settlementAmount"
            ),
          });
    ...
    methods: {
      ...
      inputChange(index, attribute) {
        ...
        if (regexp.test(this.formData.entries[index][attribute])) {
          this.formData.entries[index][attribute] = Number.parseFloat(
            this.formData.entries[index][attribute].slice(
              0,
              this.formData.entries[index][attribute].length - 1
            )
          );
        }
      },
      onAdd() {
        ...
        this.formData.entries.push(newItem);
      },
      onCopy(index, row) {
      ...
      onDelete(index, row) {
        if (row.relationId) {
          this.$confirm(
            ...
            const i = this.formData.shippingSheets.findIndex(
              (item) => item.id === row.relationId
            );
            if (i > -1) {
      ...
      onSelectSample(products) {
        this.formData.entries[this.operateIndex].product.skuID =
          products[0].skuID;
        ...
        this.formData.entries[this.operateIndex].product.thumbnail =
      ...
      _modifyColumn(column) {
        let index = this.formData.colNames.findIndex(
