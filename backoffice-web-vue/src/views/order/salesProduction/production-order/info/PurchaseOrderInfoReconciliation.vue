<template>
  <div class="info-receive-body">
    ...
    <el-form :disabled="hasPartBPermission">
      ...
      <table cellspacing="2" width="100%" :height="form.entries.length*50" class="order-table">
        <tr class="order-table-th_row">
          ...
          <template v-for="(item,index) in sizes">
            <th :key="index" :colspan="getColspanLength3()">{{item.name}}</th>
          </template>
          <th>数量小计</th>
        </tr>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getColspanLength2() {
        return this.sizes.length > 1 ? this.sizes.length : 2;
      },
      getColspanLength3() {
        return this.sizes.length > 1 ? 1 : 2;
      },
      async onSubmit(){
      ...
      async onCreate() {
        ...
        const url = this.apis().createReconciliationOrderV2(this.slotData.code);
      ...
      async onUpdate() {
        ...
        const url = this.apis().updateReconciliationOrderV2();
      ...
      async onCommit() {
        ...
        const url = this.apis().updateAndCommitReconciliationOrderV2();
        const result = await this.$http.put(url, form);
        if (result['errors']) {
          this.$message.error(result['errors'][0].message);
          return;
      ...
      async onWithdraw() {
        const url = this.apis().withdrawReconciliationOrderV2();
      ...
      async onReject() {
        const url = this.apis().rejectReconciliationV2(this.form.code);
        const result = await this.$http.put(url);
        if (result['errors']) {
          this.$message.error(result['errors'][0].message);
          return;
      ...
      async onAccept() {
        const url = this.apis().confirmReconciliationV2(this.form.code);
        const result = await this.$http.put(url);
        if (result['errors']) {
          this.$message.error(result['errors'][0].message);
          return;
      ...
      initTable() {
        ...
        this.colors.forEach(color => {
          ...
          this.sizes.forEach(size => {
            if (this.slotData.reconciliationOrders == null || this.slotData.reconciliationOrders.length == 0) {
              let variant = this.getVariant(color, size.name, this.slotData.deliveryOrders[0].entries);
              if (variant != null) {
                sizeArray.push({
                  size: size.name,
                  color: color,
              ...
              let variant = this.getVariant(color, size.name, this.slotData.reconciliationOrders[0].entries);
              if (variant != null) {
      ...
      initUpdateForm() {
        if (this.slotData.reconciliationOrders != null && this.slotData.reconciliationOrders.length != 0) {
          ...
          this.form.id = '';
          this.form.code = '';
          this.form.partA = this.slotData.partyACompany.name;
          this.form.partB = this.slotData.partyBCompany.name;
