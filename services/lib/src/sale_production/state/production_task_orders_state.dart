...
class ProductionTaskOrdersState extends PageState {
  ...
  void getOrders(String status) async {
    ...
    if (_ordersMap[status].totalElements < 0) {
      ...
      try {
        response = await http$.post(SaleProductionApis.productionTaskOrderList,
            data: data,
            queryParameters: {
      ...
      if (_ordersMap[status].currentPage + 1 != _ordersMap[status].totalPages) {
        ...
        try {
          response = await http$.post(
              SaleProductionApis.productionTaskOrderList,
              data: data,
              queryParameters: {
                'page': ++_ordersMap[status].currentPage,
                'size': _ordersMap[status].size,
