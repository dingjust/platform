...
///对账单状态管理
class ReconciliationOrdersState extends PageState {
  Map<String, PageEntry> _ordersMap = {
    'ALL': PageEntry(
        currentPage: 0,
        size: 10,
        data: List<FastReconciliationSheetModel>(),
        totalElements: -1),
    'PENDING_B_SIGN': PageEntry(
        currentPage: 0,
        size: 10,
        data: List<FastReconciliationSheetModel>(),
        totalElements: -1),
    'PENDING_APPROVAL': PageEntry(
        currentPage: 0,
        size: 10,
        data: List<FastReconciliationSheetModel>(),
        totalElements: -1),
    'PENDING_A_SIGN': PageEntry(
        currentPage: 0,
        size: 10,
        data: List<FastReconciliationSheetModel>(),
        totalElements: -1),
  ...
  List<FastReconciliationSheetModel> orders(String status, {String keyword}) {
    if (_ordersMap[status].totalElements < 0) {
      getOrders(status);
    }
  ...
  void getOrders(String status) async {
    ...
    if (_ordersMap[status].totalElements < 0) {
      ...
      try {
        response = await http$.post(OrderApis.reconciliationOrderList,
            data: data,
            queryParameters: {
      ...
      if (response != null && response.statusCode == 200) {
        ReconciliationOrderResponse ordersResponse =
            ReconciliationOrderResponse.fromJson(response.data);
  ...
  void clear() {
    _ordersMap.forEach((key, entry) {
      entry.currentPage = 0;
      entry.size = 10;
      entry.data = List<FastReconciliationSheetModel>();
