...
class SaleOrderBLoC extends BLoCBase {
  // 工厂模式
  factory SaleOrderBLoC() => _getInstance();

  static SaleOrderBLoC get instance => _getInstance();
  static SaleOrderBLoC _instance;
  ...
  SaleOrderBLoC._internal() {}

  static SaleOrderBLoC _getInstance() {
    if (_instance == null) {
      _instance = SaleOrderBLoC._internal();
    }
    return _instance;
  }
...
//  Stream<List<PurchaseOrderModel>> get purchasestream => _controller.stream;
  ...
  loadingMoreByKeyword(String keyword) async {
    //若没有数据则查询
    if (_ordersMap['SEARCH'].currentPage + 1 ==
        _ordersMap['SEARCH'].totalPages) {
      //通知显示已经到底部
